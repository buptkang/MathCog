...
/// We maintain this entire duplicate (virtually) of the Expr hierarchy 
/// because of F#'s annoying treatment of return values (requiring them to be explicitly cast 
/// to the same type even if they're all subtypes of one type). FS stands for F#
...
type public ExprTransform() = 
    ...
    static let rec fromexpr (e : Expr) = 
        // NB: Force Parentheses annotation not handled yet
        //--but this function is so far only used to back-convert operators
        match e with
        | :? WellKnownSym as wks -> WellKnownSymFS(wks.ID)
        | :? WordSym as ws -> 
            let wsfs = WordSymFS(ws.Word, ws.Annotations.Contains("Factor"))
            match ws.Subscript with
            | :? NullExpr -> wsfs
            | _ -> SubscriptFS(wsfs, fromexpr ws.Subscript)
        | :? CompositeExpr as ce -> 
            CompositeExprFS(fromexpr ce.Head, 
                            ce.Args |> Array.map fromexpr |> Array.toList, 
                            ce.Annotations.ContainsKey("Factor"))
        ...
        | :? LetterSym as ls -> 
            let lsfs = LetterSymFS(ls.Letter, ls.Annotations.ContainsKey("Factor"))
            match ls.Subscript with
            | :? NullExpr -> lsfs
            | _ -> SubscriptFS(lsfs, fromexpr ls.Subscript)
        | :? ErrorMsgExpr as eme -> ErrorMsgExprFS(eme.Msg)
        | _ -> failwith "???"
    
    /// From ExprFS to real Expr
    static let rec toexpr efs = 
        let addFactor (expr : Expr, factor) = expr
        match efs with
        | CompositeExprFS(head, args, factor) -> 
            addFactor (new CompositeExpr(toexpr head, 
                                         args
                                         |> List.map toexpr
                                         |> List.toArray), factor)
        ...
        | ArrayExprFS(arr) -> 
            new ArrayExpr(Array2D.init arr.Length (Array.map Array.length arr |> Array.max) 
                              (fun i j -> toexpr arr.[i].[j])) :> Expr
        ...
        | SubscriptFS(e, sub) -> 
            ...
            | WordSymFS(w, factor) -> 
                ...
                result :> Expr
            | _ -> 
                new CompositeExpr(WellKnownSym.subscript, 
                                  [| toexpr e
                                     toexpr sub |]) :> Expr
        | ErrorMsgExprFS(msg) -> new ErrorMsgExpr(msg) :> Expr
    
    /// shorthands for matching different types of Exprs
    static let (|SymPlus|_|) (e) = 
        match e with
        | WellKnownSymFS(WKSID.plus) -> Some()
        | _ -> None
    
    static let (|SymMinus|_|) (e) = 
        match e with
        | WellKnownSymFS(WKSID.minus) -> Some()
        | _ -> None
    
    static let (|SymEq|_|) (e) = 
        match e with
        | WellKnownSymFS(WKSID.equals) -> Some()
        | _ -> None
    
    static let (|SymTimes|_|) (e) = 
        match e with
        | WellKnownSymFS(WKSID.times) -> Some()
        | _ -> None
    
    static let (|SymDiv|_|) (e) = 
        match e with
        | WellKnownSymFS(WKSID.divide) -> Some()
        | _ -> None
    
    static let (|SymPower|_|) (e) = 
        match e with
        | WellKnownSymFS(WKSID.power) -> Some()
        | _ -> None
    
    static let (|CEXP|_|) (e) = 
        match e with
        | CompositeExprFS(h, a, f) -> Some(h, a, f)
        | _ -> None
    ...
    static let rec _FactorOut (inExpr : ExprFS, factor) = 
        match inExpr with
        // a + bx + c -> x (a/x + b + c/x)
        | CEXP(SymPlus, args, f) when containsInSum (args, factor) -> 
            plus (List.map (fun arg -> fromexpr (Engine.Simplify(toexpr (divide (arg, factor))))) args)
        // 1/(abx)  -> x / abx^2
        | CEXP(SymTimes, args, f) when containsTop (flat (args), factor) -> times (remove (flat (args), factor))
        // 1/(abx)  -> x / abx^2
        | CEXP(SymMinus, [ arg ], f) when containsTop (flat ([ arg ]), factor) -> 
            times (bint (-1) :: remove (flat ([ arg ]), factor))
        // a / x -> x a / x^2
        | CEXP(SymTimes, numerator :: [ CEXP(SymDiv, [ arg ], f2) ], f3) when arg = factor -> 
            divide (numerator, power (factor, bint (2)))
        // a/(bx)  -> x a / bx^2
        | CEXP(SymTimes, numerator :: [ CEXP(SymDiv, [ CEXP(SymTimes, args, f) ], f2) ], f3) when containsTop 
                                                                                                      (flat (args), 
                                                                                                       factor) -> 
            divide (numerator, times (power (factor, bint (2)) :: remove (flat (args), factor)))
        // 1/(ax)  -> x / ax^2
        | CEXP(SymDiv, [ CEXP(SymTimes, args, f) ], f2) when containsTop (flat (args), factor) -> 
            inverse (times (power (factor, bint (2)) :: remove (flat (args), factor)))
        // a^n -> a * a^(n-1)
        | CEXP(SymPower, bas :: (pow :: rem), f) when bas = factor -> 
            fromexpr (Engine.Simplify(toexpr (power (bas, plus (pow :: [ minus (bint (1)) ])))))
        // leave it
        | other -> other
    ...
    static let rec _MoveAcross (inExpr : ExprFS, factor) = 
        match inExpr with
        // a + b + x  = z  -> a + b  = z - x
        | CEXP(SymEq, CEXP(SymPlus, args, f) :: rhs, f2) when containsTop (args, factor) -> 
            eq (plus (remove (args, factor)) :: [ plus (minus (factor) :: rhs) ])
        // lhs  = z  -> 0  = z - lhs
    ...
    static member public FromExpr(inexpr) = fromexpr (inexpr)
